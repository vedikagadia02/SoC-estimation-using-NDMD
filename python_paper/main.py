...
def loadData(file_name, ntrain, ntest):
    ...
    training_input_data = np.concatenate([training_data[i, :endIdx, :] for i in range(0,ntrain,1)], axis=0)
    training_target_data = np.concatenate([training_data[i, :endIdx+1, :] for i in range(0, ntrain, 1)], axis=0)
